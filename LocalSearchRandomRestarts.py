...
def local_search_first_improvement():
    ...
    return x_best, f_best, solutions_checked


def main():
    best_x = None
    best_f = [-1, -1]
    num_restarts = 10  # This changes how many restarts to perform
    num_solutions = 0
    for i in range(num_restarts):
        x, f, sol = local_search_first_improvement()
        num_solutions += sol
        if f[0] > best_f[0]:
            best_x = x
            best_f = f

    print("\nFinal number of solutions checked: ", num_solutions)
    print("Best value found: ", best_f[0])
    print("Weight is: ", best_f[1])
    print("Total number of items selected: ", np.sum(best_x))
