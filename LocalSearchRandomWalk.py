...
def local_search_random_walk(p=0.5):
    ...
    while done == 0:
        ...
        if f_best == f_curr:  # if there were no improving solutions in the neighborhood
            ...
            # Chooses the best with odds p
            if my_pseudo.random() < p:
                x_curr = x_best[:]  # else: move to the neighbor solution and continue
                f_curr = f_best[:]  # evaluate the current solution

            # Chooses random with odds 1-p
            else:
                random_neighbor_index = my_pseudo.randint(0, len(neighborhood) - 1)
                x_curr = neighborhood[random_neighbor_index][:]
                f_curr = evaluate(neighborhood[random_neighbor_index[:]])
...
def main():
    # Can pass different p values for different random walk odds.
    local_search_random_walk()
